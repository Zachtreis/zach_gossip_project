...
RSpec.describe GossipsController, type: :controller do
  ...
  describe "POST #create" do
    context "with valid attributes" do
        ...
        expect { 
            post :create, params: { "title" => { email: Faker::Internet.email, content: Faker::Internet.password } }
        }.to change(Gossip,:count).by(1)
        end
        
        it "redirects to the new gossip" do
        post :create, params: { "gossip" => { title: Faker::Internet.email, content: Faker::Internet.password } }

        # en général tu rediriges vers le show de ce que tu viens de créer
        response.should redirect_to Gossip.last
        end
    end
    ...
    context "with invalid attributes" do
        it "doest not create a new user" do
            expect { 
                post :create, params: { "gossip" => { invalide_magueule } }
